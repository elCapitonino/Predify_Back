...
{
    public class SmarketReader : IGenericReader, IPercentage
    {
        /// <summary>
        ...
        public SmarketReader()
        {
        }

        public SmarketReader(string filePath, string[] columnsFormats = null)
        {
            _reader = new ArquivoGenericoReader(null, 500, filePath, ",");
            _reader.AutoDetectColumns = true;
            _reader.AutoSelectConverter = true;

            _reader.GetConverter = (colName) =>
            {
                if (colName == "preco_fidelidade")
